...
Copyright (c) 2010-2020 Multi Theft Auto
...
addCommandHandler( "createtext",
	function( player, commandName, ... )
		if ( ... ) then
			...
			outputChatBox( "Syntax: /" .. commandName .. " [texto]", player, 255, 255, 255 )
		end
	end,
	true
)
		...
		if textID then
			...
			if element then
				if exports.sql:query_free( "DELETE FROM 3dtext WHERE textID = " .. textID ) then
					outputChatBox( "Borraste el texto (ID " .. textID .. ").", player, 0, 255, 153 )
				...
				outputChatBox( "Texto no encontrado.", player, 255, 0, 0 )
			end
		else
		...
		if hasObjectPermissionTo( player, "command.createtext", false ) or hasObjectPermissionTo( player, "command.deletetext", false ) then
			...
			for key, element in pairs( elements ) do
				if getElementDimension( element ) == dimension and getElementInterior( element ) == interior then
					...
					if distance < 20 then
						outputChatBox( "Texto ID: " .. key .. ": " .. tostring( getElementData( element, "text" ) ), player, 255, 255, 0 )
					end
				end
			end
		end
