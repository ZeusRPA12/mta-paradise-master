...
function drawWaitingText( )
	...
	if vehicle then
		...
		elseif wait and wait ~= 0 then
			...
			if diff >= 0 then
				text = ( "PorFavor Espera %.1f segundos" ):format( diff / 1000 )
			else
...
addEventHandler( "onClientMarkerLeave", resourceRoot,
	function( element, matching )
		if matching and element == localPlayer then
			wait = false
			removeEventHandler( "onClientRender", root, drawWaitingText )
			exports.gui:hint( "Tu Trabajo: Coductor de Bus", "No tan rápido, a pesar de que un conductor siempre tiene prisa, debe dejar a todos alrededor el tiempo suficiente para ingresar, está bien?", 3 )
		end
	end
)
