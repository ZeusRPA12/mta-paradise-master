...
for key, value in ipairs( vehicles ) do
	...
	if model then
		...
		outputDebugString( "Vehiclo '" .. tostring( value ) .. " No encontrado." )
	end
end
vehicles = vehicles2
vehicles2 = nil
		...
		if c then
			if p[ player ].route[ p[ player ].checkpoint ] then
				...
				if p[ player ].route[ p[ player ].checkpoint ].hint then
					triggerClientEvent( player, "gui:hint", player, "Tu Job: Conductor de Bus", p[ player ].route[ p[ player ].checkpoint ].hint )
				end
			end
...
addEventHandler( "onVehicleStartEnter", root,
	function( player, seat )
		if seat == 0 and isJobVehicle( source ) and "job-" .. tostring( exports.players:getJob( player ) ) ~= getResourceName( resource ) then
			outputChatBox( "(( Tu Puedes obtener el job en el ayuntamiento. ))", player, 255, 0, 0 )
			cancelEvent( )
		end
	end
)
