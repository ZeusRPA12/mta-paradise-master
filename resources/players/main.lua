						...
						if group.groupID == group2.groupID then
							...
							if account then
								...
								if isGuestAccount( getPlayerAccount( player ) ) and not logIn( player, account, info.mtasalt ) then
									...
									-- show him a message
									outputChatBox( "Ahora ha iniciado sesión como " .. group2.displayName .. ".", player, 0, 255, 0 )
			...
			if not shouldHaveAccount and account then
				...
				for key, value in ipairs( groups ) do
					if aclGroupRemoveObject( aclGetGroup( value.aclGroup ), "user." .. info.username ) then
						...
						outputChatBox( "Ya no estás conectado como " .. group.displayName .. ".", player, 255, 0, 0 )
					end
				end
								...
								for key2, group2 in ipairs( groupinfo ) do
									if group.groupID == group2.groupID then
										...
										if aclGroupAddObject( aclGetGroup( group.aclGroup ), "user." .. accountName ) then
											...
											userChanged = true
											if player then
												outputChatBox( "Ahora ha iniciado sesión como " .. group.displayName .. ".", player, 0, 255, 0 )
											end
										end
									end
								end
...
addCommandHandler( "reloadpermissions",
	function( player )
		if aclUpdate( nil, true ) then
			outputServerLog( "Se han recargado los permisos. (hecho  by " .. ( not player and "Console" or getAccountName( getPlayerAccount( player ) ) or getPlayerName(player) ) .. ")" )
			if player then
				outputChatBox( "Se han recargado los permisos.", player, 0, 255, 0 )
			end
		else
			outputServerLog( "Falló la recarga de permisos. (hecho by " .. ( not player and "Console" or getAccountName( getPlayerAccount( player ) ) or getPlayerName(player) ) .. ")" )
			if player then
				outputChatBox( "Falló la recarga de los permisos.", player, 255, 0, 0 )
			end
		end
	end,
	true
...
local function showLoginScreen( player, screenX, screenY, token, ip )
	...
	if screenX and screenY then
		if screenX < 800 or screenY < 600 then
			kickPlayer( player, "Use 800x600 o una resolución mayor." )
			return
		end
	end
...
-- Ajuste para permitir que varios servidores ejecuten el paraíso sin interferir con los archivos de inicio de sesión.
-- esto también se usa para calcular el hash de un jugador individual, por lo que incluso los jugadores con la misma ip / serie / nombre de usuario / contraseña en dos servidores no obtendrán el mismo hash para iniciar sesión.
-- Si alguna vez es robado / abusado, ¿si es posible? -, simplemente elimine el archivo token.xml y reinicie este recurso, esto hará que cada jugador vuelva a iniciar sesión.
