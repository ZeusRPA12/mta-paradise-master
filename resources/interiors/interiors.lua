	...
	function( player, commandName, id, price, type, ... )
		if id and tonumber( price ) and tonumber( type ) and ( ... ) then
			...
			if interior then
				...
				outputChatBox( "Interior " .. id:lower( ) .. " no existe.", player, 255, 0, 0 )
			end
		else
			outputChatBox( "Syntax: /" .. commandName .. " [id] [precio] [type 0=Gov 1=Casa 2=Biz] [nombre]", player, 255, 255, 255 )
		end
	end,
	true
)
		...
		if interiorID then
			...
			if interior then
				if exports.sql:query_free( "DELETE FROM interiors WHERE interiorID = " .. interiorID ) then
					outputChatBox( "Borraste el interior (ID " .. interiorID .. ").", player, 0, 255, 153 )
	...
	function( player, commandName, id )
		if id then
			...
			if int then
				...
				outputChatBox( "No estas en ningun interior.", player, 255, 0, 0 )
			end
		else
			outputChatBox( "Syntax: /" .. commandName .. " [id]", player, 255, 255, 255 )
		end
	...
	function( player, ... )
		...
		if int then
			if hasObjectPermissionTo( player, "command.createinterior", false ) or hasObjectPermissionTo( player, "command.deleteinterior", false ) or hasObjectPermissionTo( player, "command.setinterior", false ) or hasObjectPermissionTo( player, "command.setinteriorprice", false ) then
				...
				outputChatBox( "Tu interior: " .. getElementDimension( player ), player ,255, 255, 255 )
			end
		else
			outputChatBox( "No estas en ningun interior.", player, 255, 0, 0 )
		end
		...
		if interior then
			...
			if interior.type > 0 and interior.characterID == exports.players:getCharacterID( player ) then
				...
				if exports.sql:query_free( "UPDATE interiors SET characterID = 0 WHERE interiorID = " .. interiorID ) then
					...
					-- show a message
					outputChatBox( "Vendiste esta casa por $" .. math.floor( interior.price / 2 ) .. ".", player, 0, 255, 0 )
				else
					outputChatBox( "Error MySQL.", player, 255, 0, 0 )
				end
			else
				outputChatBox( "Esta no es tu casa.", player, 255, 0, 0 )
			end
		else
			outputChatBox( "No estas en ningun interior.", player, 255, 0, 0 )
		end
			...
			if exports.players:takeMoney( player, interior.price ) then
				...
				if characterID then
					...
					if exports.sql:query_free( "UPDATE interiors SET characterID = " .. characterID .. " WHERE interiorID = " .. data.id ) then
						...
						outputChatBox( "Felicistaciones! Compraste " .. interior.name .. " por $" .. interior.price .. "!", player, 0, 255, 0 )
					else
						outputChatBox( "Error MySQL.", player, 255, 0, 0 )
				...
				outputChatBox( "Necesitas $" .. ( interior.price - exports.players:getMoney( player ) ) .. " para comprar esta propiedad.", player, 255, 0, 0 )
			end
		elseif interior.type > 0 and interior.locked then
			exports.chat:me( player, "Trata de abrir la puerta pero no lo consigue." )
		else
