...
addCommandHandler( "markteleport",
	function( player )
		...
		if hasObjectPermissionTo( player, "command.createteleport", false ) then
			...
			outputChatBox( "Posición de teletransporte marcada. [" .. table.concat( { "x=" .. x, "y=" .. y, "z=" .. z, "i=" .. interior, "d=" .. dimension }, ", " ) .. "]", player, 0, 255, 153 )
		end
	end
)
	...
	function( player, commandName )
		...
		if a then
			...
			if insertid then
				...
				outputChatBox( "Teleport creado. (ID " .. insertid .. ")", player, 0, 255, 0 )
			...
			outputChatBox( "Primero debe establecer el lugar opuesto con /markteleport.", player, 255, 0, 0 )
		end
	end,
	true
)
		...
		if teleportID then
			...
			if teleport then
				if exports.sql:query_free( "DELETE FROM teleports WHERE teleportID = " .. teleportID ) then
					outputChatBox( "Eliminaste el teletransporte " .. teleportID .. ".", player, 0, 255, 153 )
				...
				outputChatBox( "Teletransporte no encontrado.", player, 255, 0, 0 )
			end
		else
...
addCommandHandler( "nearbyteleports",
	function( player, commandName )
		if hasObjectPermissionTo( player, "command.createteleport", false ) or hasObjectPermissionTo( player, "command.deleteteleport", false ) then
			...
			outputChatBox( "Teletransportes cercanos:", player, 255, 255, 0 )
