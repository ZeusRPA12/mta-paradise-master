...
local function createShopPed( shopID )
	...
	outputDebugString( "Error al crear la tienda " .. tostring( shopID ) )
	return false
end
...
local function loadShop( shopID, x, y, z, rotation, interior, dimension, configuration, skin )
	...
	if not createShopPed( shopID ) then
		outputDebugString( "Creacion de tienda fallida: tienda " .. tostring( shopID ) )
	end
end
		...
		if config then
			if shop_configurations[ config ] then
				...
				if shopID then
					...
					outputChatBox( "Creada nueva tienda (ID " .. shopID .. "), tipo: " .. config .. ".", player, 0, 255, 0 )
					exports['job-carrier']:addDropOff( dimension )
				else
					outputChatBox( "Error MySQL.", player, 255, 0, 0 )
				end
			else
				outputChatBox( "Coloca un nombre para la tienda '" .. config .. "'.", player, 255, 0, 0 )
			end
		else
			outputChatBox( "Syntax: /" .. commandName .. " [tipo]", player, 255, 255, 255 )
		end
	end,
	true
)
		...
		if shopID then
			...
			if shop then
				if exports.sql:query_free( "DELETE FROM shops WHERE shopID = " .. shopID ) and exports.sql:query_free( "DELETE FROM shopitems WHERE shopID = " .. shopID ) then
					outputChatBox( "Borraste la tienda (ID " .. shopID .. ").", player, 0, 255, 153 )
					deleteShop( shopID )
				else
					outputChatBox( "Error MySQL.", player, 255, 0, 0 )
				end
		...
		if hasObjectPermissionTo( player, "command.createshop", false ) or hasObjectPermissionTo( player, "command.deleteshop", false ) then
			...
			for key, value in pairs( shops ) do
				if isElement( key ) and getElementDimension( key ) == dimension and getElementInterior( key ) == interior then
					...
					if distance < 20 then
						outputChatBox( "  Tienda " .. value .. " - Tipo: " .. tostring( shops[ value ].configuration ) .. ".", player, 255, 255, 0 )
					end
				end
			end
		end
					...
					if item then
						if exports.players:takeMoney( source, item.price ) then
							...
							if exports.items:give( source, item.itemID, value, item.name ) then
								outputChatBox( "Compraste un " .. ( item.name or exports.items:getName( item.itemID ) ) .. " por $" .. item.price .. ".", source, 0, 255, 0 )
								if item.itemID == 7 then
									outputChatBox( "Tu numero de telefono es #" .. value .. ".", source, 0, 255, 0 )
								end
							end
						else
							outputChatBox( "No te puedes permitir comprar " .. ( item.name or exports.items:getName( item.itemID ) ) .. ".", source, 0, 255, 0 )
						end
					end
				end
			end
