...
local function messageBox( )
	if getDistanceBetweenPoints3D( x, y, z, getElementPosition( localPlayer ) ) > 0.5 then
		...
		if getPlayerMoney( ) - getVehiclePrice( vehicle ) < 0 then
			text = "Necesitas $" .. ( getVehiclePrice( vehicle ) - getPlayerMoney( ) ) .. "\npara comprar esto " .. getVehicleName( vehicle ) .. "."
		else
			text = "Compraste un " .. getVehicleName( vehicle ) .. "\n por $" .. getVehiclePrice( vehicle ) .. ", Pulsa 'Enter'."
		end
