...
for key, value in ipairs( vehicles ) do
	...
	if model then
		...
		outputDebugString( "Vehiculos '" .. tostring( value ) .. " no existe." )
	end
end
vehicles = vehicles2
vehicles2 = nil
...
local function newDropOff( player, earnings )
	...
	if id then
		...
		outputChatBox( "(( " .. ( earnings and ( "Tu  Ganacia $" .. earnings .. ". " ) or "" ) .. "Proxima entrega:: " .. name .. " ))", player, 255, 204, 0 )
		return true
	else
				...
				if interior then
					if getDistanceBetweenPoints3D( x, y, z, getElementPosition( interior.outside ) ) < 50 then
						if exports.interiors:setDropOff( dimension, x, y, z ) then
							...
							dropOffs[ index ].y = y
							dropOffs[ index ].z = z
							outputChatBox( "Proxima Entrega: " .. interior.name .. " ha sido actualizada.", player, 0, 255, 0 )
						else
							outputChatBox( "Fallo al cambiar la entrega.", player, 255, 0, 0 )
						end
					else
						outputChatBox( "Estas demasiado lejos de la entrada.", player, 255, 0, 0 )
					end
				else
					outputChatBox( "El interior no existe", player, 255, 0, 0 )
				end
			else
				outputChatBox( "No Existe ninguna tienda en esta dimension " .. dimension .. ".", player, 255, 0, 0 )
			end
		else
			outputChatBox( "Syntax: /" .. commandName .. " [dimension] - establece la entrega para todas las tiendas en esa dimensión.", player, 255, 255, 255 )
		end
	end,
	true
)
...
addEventHandler( "onVehicleStartEnter", root,
	function( player, seat )
		if seat == 0 and isDeliveryVehicle( source ) and "job-" .. tostring( exports.players:getJob( player ) ) ~= getResourceName( resource ) then
			outputChatBox( "(( Tu Puedes trabajar este trabajo en el ayuntamiento. ))", player, 255, 0, 0 )
			cancelEvent( )
		end
	end
)
