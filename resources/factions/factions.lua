	...
	if characterID then
		...
		for key, value in ipairs( result ) do
			...
			if factions[ factionID ] then
				...
				outputDebugString( "Faccion " .. factionID .. " no existe, remueve a los jugadores de ella." )
	...
	function( )
		...
		for key, value in ipairs( result ) do
			if value.groupName then
				...
				outputDebugString( "Faccion " .. value.factionID .. " no es un grupo valido. Ignorando..." )
			end
		end
...
local function joinFaction( inviter, player, faction )
	if exports.players:isLoggedIn( player ) and p[ player ] then
		if not p[ player ].rfactions[ faction ] then
			...
			if exports.sql:query_free( "INSERT INTO character_to_factions (characterID, factionID) VALUES (" .. exports.players:getCharacterID( player ) .. ", " .. faction .. ")" ) then
				...
				if result.number == 1 then
					if not exports.sql:query_free( "INSERT INTO wcf1_user_to_groups (userID, groupID) VALUES (" .. exports.players:getUserID( player ) .. ", " .. factions[ faction ].group .. ")" ) then
						...
						outputChatBox( "(( Error MySQL. ))", inviter, 255, 0, 0 )
			...
			outputChatBox( "(( " .. getPlayerName( player ):gsub( "_", " " ) .. " ya esta en una faccion. ))", inviter, 255, 0, 0 )
		end
	end
	return false
end
	...
	function( faction )
		...
		if client and client ~= source and p[ client ] and p[ client ].rfactions[ faction ] and p[ client ].rfactions[ faction ].leader >= 1 then
			if joinFaction( client, source, faction ) then
				outputChatBox( "(( " .. getPlayerName( client ):gsub( "_", " " ) .. " te coloco en la faccion " .. factions[ faction ].name .. ". ))", source, 0, 255, 0 )
				outputChatBox( "(( Colocaste a " .. getPlayerName( source ):gsub( "_", " " ) .. " en la faccion " .. factions[ faction ].name .. ". ))", client, 0, 255, 0 )
			end
		end
	end
)
			...
			if factions[ faction ] then
				...
				if other then
					if joinFaction( player, other, faction ) then
						outputChatBox( "(( " .. getPlayerName( player ):gsub( "_", " " ) .. " te coloco en la faccion " .. factions[ faction ].name .. ". ))", other, 0, 255, 153 )
						if player ~= other then
							outputChatBox( "(( Colocaste a " .. getPlayerName( other ):gsub( "_", " " ) .. " en la faccion " .. factions[ faction ].name .. ". ))", player, 0, 255, 153 )
						end
					end
				end
			else
				outputChatBox( "Esta faccion no existe.", player, 255, 0, 0 )
			end
		else
			outputChatBox( "Syntax: /" .. commandName .. " [jugador] [faccion]", player, 255, 255, 255 )
		end
	end,
	true
)
		...
		if source == client then
			if factions[ faction ] and p[ source ].rfactions[ faction ] then
				if exports.sql:query_free( "DELETE FROM character_to_factions WHERE characterID = " .. exports.players:getCharacterID( source ) .. " AND factionID = " .. faction .. " LIMIT 1" ) then
					sendMessageToFaction( faction, "(( " .. getPlayerName( source ):gsub( "_", " " ) .. " se salio de la faccion " .. factions[ faction ].name .. ". ))", 255, 127, 0 )
		...
		if source == client and p[ source ].rfactions[ faction ] and p[ source ].rfactions[ faction ].leader == 2 and type( name ) == "string" then
			...
			if player ~= source then -- You can't change your own rights.
				...
				if exports.sql:query_affected_rows( "UPDATE character_to_factions cf, characters c SET cf.factionLeader = cf.factionLeader - 1 WHERE c.characterID = cf.characterID AND c.characterName = '%s' AND cf.factionLeader >= 0", name ) == 1 then
					sendMessageToFaction( faction, "(( " .. factions[ faction ].tag .. " - " .. getPlayerName( source ):gsub( "_", " " ) .. " degrado a " .. name .. " hacia " .. ( rightNames[ new ] or "Miembro" ) .. ". ))", 255, 127, 0 )
					if player then
		...
		if source == client and p[ source ].rfactions[ faction ] and p[ source ].rfactions[ faction ].leader == 2 and type( name ) == "string" then
			...
			if player ~= source then -- You can't change your own rights.
				...
				if exports.sql:query_affected_rows( "UPDATE character_to_factions cf, characters c SET cf.factionLeader = cf.factionLeader + 1 WHERE c.characterID = cf.characterID AND c.characterName = '%s' AND cf.factionLeader < 2", name ) == 1 then
					sendMessageToFaction( faction, "(( " .. factions[ faction ].tag .. " - " .. getPlayerName( source ):gsub( "_", " " ) .. " promovio a " .. name .. " hacia " .. ( rightNames[ new ] or "Miembro" ) .. ". ))", 255, 127, 0 )
					if player then
						p[ player ].rfactions[ faction ].leader = p[ player ].rfactions[ faction ].leader + 1
					end
				else
		...
		if source == client and p[ source ].rfactions[ faction ] and p[ source ].rfactions[ faction ].leader >= 1 and type( name ) == "string" then
			...
			if factions[ faction ].ranks[ new ] then
				if exports.sql:query_affected_rows( "UPDATE character_to_factions cf, characters c SET cf.factionRank = cf.factionRank - 1 WHERE c.characterID = cf.characterID AND c.characterName = '%s' AND cf.factionRank > 1", name ) == 1 then
					sendMessageToFaction( faction, "(( " .. factions[ faction ].tag .. " - " .. getPlayerName( source ):gsub( "_", " " ) .. " degrado a " .. name .. " hacia " .. ( factions[ faction ].ranks[ new ] or "?" ) .. ". ))", 255, 127, 0 )
				else
					outputChatBox( "(( Error MySQL. ))", source, 255, 0, 0 )
				end
			end
		...
		if source == client and p[ source ].rfactions[ faction ] and p[ source ].rfactions[ faction ].leader >= 1 and type( name ) == "string" then
			...
			if factions[ faction ].ranks[ new ] then
				if exports.sql:query_affected_rows( "UPDATE character_to_factions cf, characters c SET cf.factionRank = cf.factionRank + 1 WHERE c.characterID = cf.characterID AND c.characterName = '%s' AND cf.factionRank < " .. #factions[ faction ].ranks, name ) == 1 then
					sendMessageToFaction( faction, "(( " .. factions[ faction ].tag .. " - " .. getPlayerName( source ):gsub( "_", " " ) .. " promovio a " .. name .. " hacia " .. ( factions[ faction ].ranks[ new ] or "?" ) .. ". ))", 255, 127, 0 )
				else
					outputChatBox( "(( Error MySQL. ))", source, 255, 0, 0 )
				end
			end
		...
		if source == client and p[ source ].rfactions[ faction ] and p[ source ].rfactions[ faction ].leader >= 1 and type( name ) == "string" then
			...
			if player ~= source then -- You can't change your own rights.
				...
				if exports.sql:query_affected_rows( "DELETE cf FROM character_to_factions cf LEFT JOIN characters c ON c.characterID = cf.characterID WHERE cf.factionID = " .. faction .. " AND c.characterName = '%s' AND cf.factionLeader < " .. p[ source ].rfactions[ faction ].leader, name ) == 1 then
					sendMessageToFaction( faction, "(( " .. factions[ faction ].tag .. " - " .. getPlayerName( source ):gsub( "_", " " ) .. " expulso a " .. name .. ". ))", 255, 127, 0 )
					if player then
						-- remove him from the tables
						p[ player ].types = { }
	...
	function( faction, ranks )
		...
		if source == client and p[ source ].rfactions[ faction ] and p[ source ].rfactions[ faction ].leader == 2 and type( ranks ) == "table" and #ranks <= maxRanks then
			...
			if #ranks > 0 then
				...
				-- message
				sendMessageToFaction( faction, "(( " .. factions[ faction ].tag .. " - " .. getPlayerName( source ):gsub( "_", " " ) .. " actualizo los rangos. ))", 255, 127, 0 )
			end
		end
	end
)
	...
	function( player, commandName, type, ... )
		if type and (...) then
			if factionTypes[ type ] then
				...
				if group then
					...
					for key, value in pairs( factions ) do
						if value.group == group.groupID then
							outputChatBox( "Este grupo ya es una faccion.", player, 255, 0, 0 )
							return
						end
					end
					...
					if faction then
						...
						-- show a message
						outputChatBox( "Faccion Creada " .. name .. " (" .. faction .. ").", player, 0, 255, 153 )
					else
				...
				outputChatBox( "La faccion solo puede ser de los siguentes tipos:", player, 255, 255, 255 )
				outputChatBox( "  " .. table.concat( list, ", " ), player, 255, 255, 255 )
			end
		else
			outputChatBox( "Syntax: /" .. commandName .. " [tipo] [nombre]", player, 255, 255, 255 )
		end
	end,
	true
)
...
addCommandHandler( "setfactionrights",
	function( player, commandName, other, faction, level )
		...
		if level and level >= 0 and level <= 2 and other and faction then
				...
				if other then
					if p[ other ] and p[ other ].rfactions[ faction ] then
						if exports.sql:query_free( "UPDATE character_to_factions SET factionLeader = " .. level .. " WHERE factionID = " .. faction .. " AND characterID = " .. exports.players:getCharacterID( other ) ) then
							...
							outputChatBox( "(( Colocaste a " .. getPlayerName( other ):gsub( "_", " " ) .. " en " .. ( rightNames[ level ] or "Miembro" ) .. " de " .. factions[ faction ].name .. ". ))", player, 0, 255, 153 )
						else
							outputChatBox( "Error MySQL.", player, 255, 0, 0 )
						end
					else
						outputChatBox( "El jugador no esta en la faccion.", player, 255, 0, 0 )
					end
				end
			else
				outputChatBox( "Esta faccion no existe.", player, 255, 0, 0 )
			...
			outputChatBox( "Syntax: /" .. commandName .. " [jugador] [faccion] [level: 0=miembro, 1=lider, 2=dueño]", player, 255, 255, 255 )
		end
	end,
	true
)
addCommandHandler( "setfactionrank",
	function( player, commandName, other, faction, rank )
		...
		if rank and rank >= 1 and other and faction then
					...
					if p[ other ] and p[ other ].rfactions[ faction ] then
						if rank <= #factions[ faction ].ranks then
							if exports.sql:query_free( "UPDATE character_to_factions SET factionRank = " .. rank .. " WHERE factionID = " .. faction .. " AND characterID = " .. exports.players:getCharacterID( other ) ) then
								outputChatBox( "(( Colocaste a " .. getPlayerName( other ):gsub( "_", " " ) .. " en " .. factions[ faction ].ranks[ rank ] .. " de " .. factions[ faction ].name .. ". ))", player, 0, 255, 153 )
							else
								outputChatBox( "MySQL-Error.", player, 255, 0, 0 )
							end
						else
							outputChatBox( "Esta faccion no tiene el rango " .. rank .. ".", player, 255, 0, 0 )
						end
					else
						outputChatBox( "El jugador no esta en la faccion.", player, 255, 0, 0 )
					end
			...
			outputChatBox( "Syntax: /" .. commandName .. " [jugador] [faccion] [rango]", player, 255, 255, 255 )
		end
	end,
	true
)
