...
local function renderFuelStation( )
	...
	if vehicle and getVehicleOccupant( vehicle ) == getLocalPlayer( ) and doesVehicleHaveFuel( vehicle ) then
		...
		if getVehicleEngineState( vehicle ) then
			refilling = 0
			dxDrawText( "Tienes que apagar el motor del vehiculo.", screenX - 150, screenY - 100, screenX - 20, 20, tocolor( 255, 255, 255, 255 ), 0.5, "bankgothic", "center" )
		else
			if fuel + refilling < 100 and getPlayerMoney( ) >= math.ceil( refilling * 0.25 ) then
				dxDrawText( "Manten 'Espacio'\npara cargar el vehiculo.", screenX - 150, screenY - 100, screenX - 20, 20, tocolor( 255, 255, 255, 255 ), 0.5, "bankgothic", "center" )
			end
			
			if refilling > 0 then
				dxDrawText( "Costo: $" .. math.ceil( refilling * 0.25 ), screenX - 150, screenY - 65, screenX - 20, 20, tocolor( 255, 255, 255, 255 ), 0.5, "bankgothic", "center" )
			end
